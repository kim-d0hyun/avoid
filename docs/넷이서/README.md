...
| `solve.py` | 풀이를 한 걸음씩 지형에 대 보는 검사. 열쇠는 닿으면 집고(지나가려면 `hop`), 포탈은 지나가면 들어가고, 내려뛰기는 발판을 뚫지 못하고, 손잡기 곁에 다른 사람이 있으면 그쪽이 끌려온다 |
...
| `build.py` | 아티팩트 페이지 (`python3 build.py` → `coop-plan6.html`) |
