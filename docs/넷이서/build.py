...
SHAPES = [('one','한 화면','36 × 22. 카메라가 안 움직인다. 「상자 계단」「빨간 열쇠」'),
          ('long','가로로 긴 한 층','108 × 22. 오른쪽으로 가면 흐른다. 「표지판」「지키는 사람」「움직이는 발판」「동시에」'),
...
<section>
  ...
  <h2>끝나는 조건 · 죽음 · 되감기</h2>
  {verbs([
    ('넷이 다 모여야 끝','출구 포탈에 넷이 들어와 있으면 켜지고 아무나 ⌥↑. 흩어져 있으면 「2명 더」가 포탈 위에 뜬다.','기본. 열두 판 중 여덟.'),
    ...
    ('시간 제한은 마지막만','120초. 다 되면 방장이 되감은 것과 같다.','시간은 가장 싼 손잡이라 한 번만 쓴다.'),
  ])}
</section>
...
<section>
  ...
  <div class="scroll"><table>
    ...
    {rows([('한 칸','42px','화면 36 × 22 칸'),('사람','0.8 × 1.2칸 · 판정 1 × 2','네모'),('점프','두 칸 위 · 네 칸 옆','세 칸 턱부터 어깨나 상자'),
           ('어깨','한 명당 +1칸','셋이 쌓이면 다섯 칸 — 「빨간 열쇠」의 기둥'),('웅크리기','0.7칸','한 칸 굴'),('걷기 · 밀기','초당 4.5 · 2.2칸','X 는 둘이 붙어야 2.2'),
  ...
  <div class="scroll"><table>
    ...
    {rows([('⌥ ← →','걷기','사다리 위에서는 안 먹는다'),('⌥ ↑','점프 · 사다리 오르기 · <b>출구 포탈</b>','포탈 위면 다음 판'),('⌥ ↓','웅크리기 · 사다리 내리기',''),
...
<section>
  ...
  <div class="steps">{''.join(f'''<div class="step"><header><h3>{t}</h3><time>{d}</time></header><ul>{"".join(f"<li>{x}</li>" for x in xs)}</ul></div>''' for t, d, xs in [
    ...
    ('2차 — 학교', '3일', ['사다리 · 리프트 · 왕복 발판(길) · 스프링', '두 층 판 · 위로 긴 판', '학교 네 판']),
    ('3차 — 도시', '3일', ['3층 판 · 통(구멍에서 나와 굴러가고 떨어지고 부서진다) · 밀려나기', '발밑이 사라질 때 떨어지기 · 상자가 떨어져 누름판 누르기', '시간 제한 · 단계 기록 · 되감기 횟수', '도시 네 판 · 넷이 한 바퀴 돌아 치수 고치기']),
  ])}</div>
</section>
