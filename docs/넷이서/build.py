...
<section>
  ...
  {verbs([
    ...
    ('넷 세상 검사(coop-net.mjs)','방장 1 + 손님 3, 세상 넷, 꾸러미를 50ms 지연으로 주고받는다. 손님이 자기 물리로 걷고, 방장 스냅샷(pack/unpack)으로 상자·열쇠·삭은 발판을 받고, 손잡기·밀기·출구 부탁이 방장을 거쳐 중계된다. 한 명씩 하는 동작(걷기·사다리·리프트·열쇠·포탈·누름판·손잡기)은 지연을 넣어도 넘어간다.','다만 봇은 한 명씩 움직인다 — 둘이 한 상자를 동시에 밀거나 사람 계단을 쌓는 진짜 동시 동작은 넷이 붙어야 안다.'),
  ])}
</section>
