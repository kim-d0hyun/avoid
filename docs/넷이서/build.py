...
LEGEND = [
    ...
    ('c / C','집결판과 집결문','살아 있는 전원이 c 위에 0.3초 서면(상자는 안 센다) C 가 영구히 열리고 <b>그 자리가 체크포인트</b> — 죽으면 거기서 다시 산다. 되감기는 지운다','#2f9c9c'),
    ('t / T','10초 스위치와 시한문','t 를 밟으면 T 가 10초 열린다. 다시 밟으면 처음부터 10초. 시간이 다 됐는데 안에 사람이 있으면 닫히지 않는다','#d97b1f'),
    ('l / L','구조 레버와 접이식 사다리','l 을 밟으면 점선으로만 보이던 L 이 사다리가 되어 그대로 남는다 — 먼저 올라간 사람이 뒤를 데려온다','#2f9c9c'),
    ('k / K · e / E','사람 수 누름판','서로 다른 사람 둘(k)·셋(e) 이상이 서 있는 동안 문이 열린다. 상자는 안 센다','#3f8f56'),
    ('p 둘','교대문','누름판을 문 양쪽에 둘 — 어느 쪽이 눌려도 셔터가 열린다. 건너간 사람이 이어받는다','#3f8f56'),
    ('i / I','탑 감지기와 문','전원이 층층이 쌓은 탑의 <b>꼭대기 사람</b>이 닿으면 문이 열린 채 남는다. 혼자 뛰어 스치는 건 안 켜진다','#2f9c9c'),
    ('V · N','승강기','세 칸 폭 발판. V 는 둘, N 은 넷이 다 서면 위 승강장으로. 비면 도로 내려온다','#8a5bb5'),
    ('J · j','분기 벨트','누름판 p(J)·q(j)가 눌린 동안 오른쪽, 아니면 왼쪽(회수)으로 돈다','#d97b1f'),
    ('z · d f g · Z','신호탑과 신호 버튼','신호탑의 모양(세모·동그라미·네모)을 멀리 있는 버튼에서 ⌥↓ 로 누르면 신호문이 열린다. 틀리면 신호가 바뀐다','#d02f22'),
...
<header class="head">
  <div class="kicker">몰겜 · 세 번째 게임 · 기획 10판 — 열다섯 판을 다시 짰다</div>
  <h1>넷이서</h1>
  <p class="lede"><b>뒷마당 · 학교 · 도시 · 지하철 · 환승역.</b> 판 열다섯, 모양도 규칙도 다 다르다 — 한 화면짜리, 가로로 긴 것, 위로 긴 것, 두 복도, 3층, 무빙워크 통로, 집결 체크포인트가 있는 2층 환승역.
     어떤 판은 <b>한 명만</b> 닿으면 끝나고 어떤 판은 <b>넷이 다</b> 모여야 끝난다.
     판마다 협동 마디가 넷 이상 — 어깨와 손, 사람 계단, 누름판 지키기, 깜빡이는 다리, 한 명씩 건너는 삭은 발판, 포탈로 보내는 상자.
     열다섯 판 풀이 {STEPS_ALL}걸음을 지형에 대 봤고, <b>같은 풀이를 실제 엔진에서 넷이 키를 눌러 전부 깼다</b> (아래 12).</p>
  <dl class="spec">
    <div><dt>인원</dt><dd>넷 고정</dd></div>
    <div><dt>판</dt><dd>5단계 · 열다섯 판 · 크기는 판마다</dd></div>
    <div><dt>끝나는 조건</dt><dd>판마다 「한 명」 / 「넷」</dd></div>
    <div><dt>죽으면</dt><dd>시작 자리에서 다시 (집결 체크포인트를 지났으면 거기서) · 방장이 ⌥R 로 판을 되감을 수 있다</dd></div>
    ...
    <div><dt>8판에서</dt><dd><b>열네 판을 전부 다시 짰다</b> — 더 어렵게, 판마다 함정 6~8개, 세계마다 이어서 해야 하는 마디 둘 이상 · 뒤 다섯 판에 시간 제한 · 검사기 1301걸음 · 엔진 봇 14/14 · 동시 시뮬 14/14 · 규약 10</dd></div>
    <div><dt>10판에서</dt><dd><b>열다섯 판을 전부 다시 짰다</b> — 새 장치 넷(사람 수 누름판 · 탑 감지기 · 승강기 · 분기 벨트 · 신호탑)과 교대문 · 상자 정차대 · 집결 무리 · 판 전체 시간 제한 없앰 · 엔진 버그 셋 · 편도 5프레임에서도 전부 통과 (아래 17)</dd></div>
    <div><dt>9판에서</dt><dd><b>새 장치 셋</b> — 전원 집결 체크포인트(c/C) · 10초 시한문(t/T) · 접이식 구조 사다리(l/L) · 그걸 엮은 환승역 「마지막 열차」 · 판 시작·되감기·부활의 초기화와 동기화를 고쳤다 · 규약 12 (아래 16)</dd></div>
  </dl>
</header>
...
<section>
  ...
  {verbs([
    ...
    ('죽으면 시작 자리에서','가시·연못에 닿으면 그 사람만 시작 자리에서 다시 산다. 집결 체크포인트(16)를 지났으면 자기 집결판에서. 판은 그대로 돈다.','팀이 이미 협동으로 건너간 자리면 혼자서는 못 따라간다 — 그래서 아래가 있다.'),
    ...
    ('시간 제한은 없다 (10판)','판 전체 시간 제한은 모두 뺐다. 박자는 10초 시한문과 깜빡이 발판이 준다. 긴 판은 집결 체크포인트로 끊는다.','처음 하는 넷이 의논하고 실패하는 시간을 제한시간이 다 잡아먹었다 — 기록 경쟁은 따로 두면 된다.'),
  ])}
</section>
...
<section>
  ...
  <p class="sub">맵은 가로로 스크롤된다. 진한 것이 땅, 갈색이 상자, 빨강·노랑·파랑 네모가 색 블록(같은 색 열쇠로 없앤다), 초록이 스위치·누름판·셔터,
     파란 점선이 왕복 발판의 길, 보라 타원이 출구, 청록이 집결판·구조 레버와 그 문·사다리, 주황 판이 시한 스위치. 네 네모가 시작 자리. 연필색 글은 배경 소품.</p>
  {''.join(cards)}
</section>
...
<section>
  ...
  <h3>확인</h3>
  {verbs([
    ('칸 검사','열다섯 판 {STEPS_ALL}걸음. 스위치·누름판이 발판을 내는 규칙을 검사기가 안다 — 누름판이 풀리면 발판이 사라진다.',''),
    ('실제 엔진','같은 풀이를 넷이 키를 눌러 열다섯 판 다 깼다 (위 12 표). 손잡기 걸음은 봇에서도 뺐다.','한 명씩 · 인형 세상. 같은 칸에 넷이 차례로 내려서면 머리 위에 쌓이므로 내려선 사람은 바로 비켜 서는 걸음을 넣었다.'),
...
<section>
  ...
  <h2>8판 — 다시 짠 열네 판</h2>
  <p class="sub">「전체적으로 더 난이도 있고, 트릭이나 협동이 많도록 재설계. 한 스테이지가 촘촘하고 다양하게 길어도 돼. 연속적으로 동작이 되어야만 하는 것도.」 이름·세계는 그대로 두고 지형과 풀이를 전부 새로 짰다. 판마다 함정 6~8개, 풀이 47~139걸음 (위 12 표).</p>
  <h3>이어서 해야 하는 마디 — 세계마다 둘 이상</h3>
  {verbs([
    ('뒷마당','표지판: 4번이 누름판을 밟는 동안만 가시 위 다리(M)가 산다 → 셋이 다리와 삭은 판자를 한 명씩 → 계단·선반 왕복 → 스위치가 셔터와 가시 위 발판을 내야 밟던 사람이 사다리로 온다. 상자 계단: 땅 상자로 첫 단 → 단 위 상자를 누름판에 → 다락에서 둘이 무거운 상자를 둘째 누름판에 → 파란 열쇠(발밑 삭제) → 되돌아 깜빡이는 다리.','누름판에서 내려서면 가시. 열쇠는 순서를 틀리면 막힌다.'),
    ('학교','엘리베이터: 리프트 → 무거운 상자로 셔터 → 삭은 통로 한 명씩 → 어깨로 한 명만 4층 → 스위치로 셔터를 열어 셋이 따라옴 → 4층 상자를 구멍에 밀어 세 층 아래 로비 누름판까지. 되돌아오기: 어깨로 셋 → 한 명이 누름판 고정 → 삭은 통로 → 끝 스위치가 1층 셔터(돌아오는 길) → 열쇠 → 1층에서 무거운 상자를 누름판에 올려 가시 다리 → 사다리 둘.','누름판을 밟던 사람이 늦게 따라온다 — 그 사람의 길을 스위치가 낸다.'),
    ('도시','3층 창고: 상자 낙하 → 상자로 누름판 → 깜빡이는 다리+삭은 통로 → 한 명이 q 를 밟는 동안만 나는 발판으로 단 위 열쇠 → 밟던 사람이 내려오는 순서. 옥상: 무거운 상자 → 삭은 통로/깜빡이는 다리 → 빨간 열쇠 → 선반 소멸 → 상자 낙하 → 누름판 → 2층 셔터 → 스위치 → 1층 끝 계단. 동시에: 누름판 둘 동시에 → 이중 셔터 → 어깨 → 트램펄린 → 스위치가 방음벽과 이중 셔터 위 계단을 함께 낸다.','도시부터 시간 제한 (160~180초, 동시에 하면 추정치의 4배 이상 여유).'),
    ('지하철','무빙워크: 어깨로 넘긴 하나의 스위치 → 셋의 발판 → 상자를 역방향 벨트에 실어 턱까지 → 누름판 → 어깨로만 닿는 포탈 → 승강장 상자 → 둘째 누름판 → 트램펄린 → 둘째 포탈. 종점: 1층 누름판이 2층 발판을 냄 → 삭은 통로/역벨트/깜빡이는 다리 → 3층 무거운 상자를 둘이 포탈에 → 1층 끝 누름판에 낙하 → 출구 셔터, 동시에 빨간 열쇠가 2층 벽을 지운다.','누름판 위 상자를 치우면 2층 발판이 사라진다.'),
  ])}
  <h3>줄인 것 · 한계</h3>
  {verbs([
    ('걸음 60 미만 세 판','두 열쇠 55 · 지키는 사람 47 · 두 길 59. 지키는 사람은 「한 명만」 판이라 사람당 걸음이 적다. 함정 수(6~8)는 채웠다.',''),
    ('탑 2층 누름판은 상자가 밟는다','사람이 밟으면 구멍 반대편에 갇히는데, 탑은 층마다 사다리가 하나(지그재그가 정체성)라 돌아올 둘째 길이 없다.',''),
    ('동시 시뮬에서 처음 여섯 판이 깨졌다','지형이 아니라 넷이 동시에 움직이는 순서 문제 — 삭은 판자에 둘이 동시에 오르고, 트램펄린에 겹쳐 서고, 누름판을 밟던 사람이 남이 건너는 중에 내려섰다. 풀이에 need_plate 를 동기 지점으로 끼워 한 명씩 지나가게 했다. 지도는 안 약해졌다.','14/14.'),
    ('엔진 한계 둘 (엔진은 손대지 않았다)','2층 바닥에서 리프트 칸으로 걸어 들어갈 수 없다(리프트는 매달리는 칸) — 내려가는 사다리를 따로 뒀다. 사다리는 위층 바닥을 뚫고 놓아야 꼭대기에서 올라선다.',''),
    ('장치 상한','스위치 a · 누름판 p·q 가 판당 하나씩, 색 열쇠 셋, 포탈 두 쌍. 긴 판은 같은 누름판을 여러 마디에서 다시 쓴다.','새 타일은 필요 없었다.'),
  ])}
</section>

<section>
  <div class="num">16</div>
  <h2>9판 — 새 장치 셋과 환승역 「마지막 열차」</h2>
  <p class="sub">「한 스테이지에 협동 요소가 여러 개. 내가 남의 길이 되고, 그 사람이 돌아와 내 길을 열어 주는 것.」 8판까지는 장치가 스위치·누름판·열쇠뿐이라
     긴 판을 짜면 무관한 구간끼리 작동이 엮이고, 한 명이 죽으면 전원이 처음부터였다. 장치 셋을 엔진에 더하고, 그걸 한 판에 엮은 환승역을 열다섯째 판으로 넣었다.
     기존 열네 판의 지형과 풀이는 손대지 않았다.</p>
  {verbs([
    ('전원 집결 체크포인트 c / C','살아 있는 전원이 청록 집결판 c 위에 0.3초 서면(상자는 안 센다) 집결문 C 가 영구히 열리고, 그 순간 각자의 자리가 체크포인트가 된다. 그 뒤로는 죽어도 시작 자리가 아니라 자기 집결판에서 다시 산다 — 긴 판에서 「한 명 죽으면 전원 처음부터」를 끊는다. 방장의 ⌥R 되감기는 체크포인트도 지운다.','부활 화면도 집결 자리를 가운데 둔다. 시작 화면은 출발 자리 한가운데 — 넷이 같은 그림으로 시작한다.'),
    ('10초 시한문 t / T','스위치 t 를 밟으면 문 T 가 10초 열린다. 다시 밟으면 10초부터 다시. 시간이 다 됐는데 문 칸 안에 사람이 있으면 닫히지 않는다 — 문에 끼는 실패는 없다.','누름판(밟는 동안)과 스위치(영구) 사이의 것. 「밟고 달려가야 하는」 박자가 생긴다.'),
    ('접이식 구조 사다리 l / L','레버 l 을 밟으면 점선으로만 보이던 L 이 사다리가 되어 그대로 남는다. 먼저 올라간 한 명이 뒤에 남은 셋을 데려오는 장치 — 어깨를 내준 사람이 그 자리에 갇히지 않는다.','n 은 발판 한 칸이라 층을 잇지 못했다. 이건 위층에서 아래층까지 한 줄로 내려온다.'),
    ('환승역 「마지막 열차」 144×36 · 넷 모두','발전기 → 어깨로 한 명 → 스위치 계단 → 집결① → 신호실(삭은 통로·10초 문·점멸) → 구조 사다리 → 화물 분기(분기 벨트) → 넷이 타는 승강기 → 옥상에서 셋이 사람 계단으로 한 명을 신호탑 선반에 → 먼 신호 버튼 → 집결② → 넷이 쌓는 탑 → 출구.','역할을 네 번 넘겨받는다. 10판에서 옥상 받침 차례를 승강기에서 내린 순서에 맞춰 고쳤다 — 전엔 봇이 선반에 못 올랐는데도 조용히 넘어갔다.'),
    ('깰 수 있다','칸 검사(run.py) → 순차 인형 풀이(coop-play) → 참가자별 독립 세상 동시 봇 → 사람 충돌 켬 → 사람 충돌 켬 + 편도 5프레임. 다섯 가지 모두 통과.','동시 봇은 10판에서 더 엄격해졌다 — 협동 마디를 모두의 세상에서 확인하고, 뛰기·어깨 착지를 재고, 못 하면 그 자리에서 실패한다.'),
    ('같이 고친 것','되감기 직후 옛 팀원 위치가 스위치를 켜던 것 · 이전 판의 좌표·밀기·출구 메시지가 새 판에 적용되던 것(판 세대 ep · 순서 sq) · 시작 메시지를 놓친 손님의 복구 · 삭은 발판이 한 사람 양발을 두 번 세던 것 · 겹친 동료에게 걸으면 뒤로 밀리던 것 · 방장이 옮긴 상자 위의 손님이 따라가지 못하던 것 · 시작 카메라와 배경 시계를 넷이 같게.','규약 12. 자세한 것은 docs/협동-업그레이드.md.'),
  ])}
<section>
  <div class="num">17</div>
  <h2>10판 — 열다섯 판을 다시 짰다</h2>
  <p class="sub">「한 스테이지에 협동 요소가 아주 많이. 판이 길어도 좋다. 대신 깰 수 있어야 한다.」 새 장치 넷을 엔진에 넣고 열다섯 판을 모두 다시 짰다.
     판 이름·세계·순서는 그대로다. 판별 요약은 <code>docs/협동-업그레이드.md</code> 5장.</p>
  {verbs([
    ('사람 수 누름판 k / K · e / E','서로 다른 사람 둘(k)·셋(e) 이상이 서 있는 동안 문이 열린다. 상자는 안 센다. 판에 「2명」「3명」.','문 양쪽에 깔면 두 레인이 인원을 나눠 잡는다.'),
    ('교대문 — 누름판 둘','누름판 p·q 를 한 판에 둘까지. 어느 쪽이 눌려도 셔터가 열린다 — 건너간 사람이 반대편 판을 이어받아 지키던 사람을 부른다.','「지키는 사람」은 결국 건너온다.'),
    ('탑 감지기 i / I','넷이 층층이 쌓은 탑의 꼭대기 사람이 벽의 눈에 닿으면 문이 열린 채 남는다. 혼자 뛰어 스치면 안 켜진다.','문은 천장까지 — 탑에서 뛰어넘을 수 없다.'),
    ('승강기 V · N','세 칸 폭 발판. V 는 둘, N 은 넷이 다 서면 오른다. 비면 내려오고, 오르다 정원이 모자라면 떨어뜨리지 않고 돌아간다.','엘리베이터·탑·동시에·옥상·종점·마지막 열차.'),
    ('분기 벨트 J · j · 상자 정차대','누름판이 눌린 동안 오른쪽, 아니면 회수 쪽으로 도는 벨트. 누름판 칸에 든 상자는 거기 선다.','한 명이 방향을 잡고 둘이 싣는다 — 정차한 상자가 그 사람 일을 넘겨받는다.'),
    ('신호탑 z · d f g · Z','신호탑의 모양(세모·동그라미·네모)을 멀리 있는 버튼에서 ⌥↓ 로. 틀리면 신호가 바뀐다.','마지막 열차 — 선반 위 사람이 말로 전해야 한다.'),
    ('판 전체 시간 제한 없앰','도시·지하철의 150~180초 제한을 모두 뺐다.','박자는 10초 시한문과 깜빡이 발판이.'),
    ('엔진 버그 셋','넷이서에서 배구로 갈아 끼우면 네트가 화면 밖 · 지연 속 무거운 상자가 안 밀림 · 사람 계단에서 가운데 받침이 밑에서 뛰는 머리에 들림.','뒤 둘은 사람이 실제로 겪는 것이다. 회귀 시험을 넣었다.'),
    ('깰 수 있다','열다섯 판 모두 칸 검사 · 인형 · 동시 봇 · 사람 충돌 · 사람 충돌 + 편도 5프레임 통과.','동시 봇이 숨기던 실패 둘(무도회장·마지막 열차)을 드러내 풀이를 고쳤다.'),
  ])}
</section>
...
<footer>
  ...
  <div>열다섯 판은 조각 DSL 로 쌓았고, 풀이 {STEPS_ALL}걸음을 지형에 한 걸음씩 대서 확인한 뒤 실제 엔진에서 키를 눌러 다시 깼다. 자료는 리포 <code>docs/넷이서/</code>, 실전 검사는 <code>test/coop-play.mjs</code>.</div>
</footer>
</div>
...
function draw(cv, st) {{
  ...
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {{
    ...
    if (ch === 'H' || ch === '|' || ch === 'L') {{ g.fillRect(X+1, Y, 2, T); g.fillRect(X+T-3, Y, 2, T); g.fillRect(X+1, Y+T/2-1, T-2, 2); continue; }}
    ...
    if (ch === 'V' || ch === 'N') {{ g.setLineDash([2,2]); g.strokeStyle = COL[ch]; g.beginPath(); g.moveTo(X-T+1, Y); g.lineTo(X-T+1, Y+T); g.moveTo(X+2*T-1, Y); g.lineTo(X+2*T-1, Y+T); g.stroke(); g.setLineDash([]); continue; }}
    if (ch === 'J' || ch === 'j') {{ g.fillRect(X, Y, T, T*0.45); continue; }}
    if ('dfgz'.includes(ch)) {{ g.beginPath(); if (ch === 'd') {{ g.moveTo(X+T/2, Y+1); g.lineTo(X+T-1, Y+T-1); g.lineTo(X+1, Y+T-1); }} else if (ch === 'f' || ch === 'z') g.arc(X+T/2, Y+T/2, T*0.4, 0, Math.PI*2); else g.rect(X+1.5, Y+1.5, T-3, T-3); g.fill(); continue; }}
    if (ch === 'i') {{ g.beginPath(); g.arc(X+T/2, Y+T/2, T*0.42, 0, Math.PI*2); g.fill(); g.fillStyle = paper; g.beginPath(); g.arc(X+T/2, Y+T/2, T*0.16, 0, Math.PI*2); g.fill(); continue; }}
    ...
    if ('apqctlke'.includes(ch)) {{ g.fillStyle = paper; g.fillRect(X+2, Y+T*0.55, T-4, T*0.3); }}
  }}
