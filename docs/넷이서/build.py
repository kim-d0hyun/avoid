...
PATTERNS = [
    ('어깨로 한 명만','세 칸 턱은 혼자 못 넘는다. 한 명이 어깨를 내주면 <b>한 명만</b> 넘어간다 — 그 한 명이 저편에서 스위치를 밟아 남은 셋의 길을 낸다. 끌어올리기(손잡기)는 7판에서 뺐다 (아래 14).','표지판 · 두 열쇠 · 무빙워크'),
    ('한 명이 낸 길','스위치를 밟으면 <b>다른 데 발판이 난다</b>(n — 그대로 남는다). 누름판은 <b>밟고 있는 동안만</b> 발판을 낸다(M·m) — 밟던 사람이 내려서면 그 위 사람이 떨어진다. 위층에서 해낸 일이 아래층 둘의 길을 만든다.','옥상(n) · 무빙워크(n) · 3층 창고(m) · 종점(m — 1층 상자가 2층 발판을 낸다)'),
    ('사람 계단','둘이 층층이 걸쳐 <b>서면</b> 계단이 된다 (선 머리는 한 칸 남짓, 웅크린 머리는 그 반). 그 위를 밟고 올라가 뛴다. 7판에서는 이걸 요구하는 판이 없다 — 규칙은 남아 있어 아무 데서나 쓸 수 있다.','(자유)'),
    ('열쇠 심부름','한 명이 팀의 도움으로 열쇠까지 간다. 집는 순간 그 색 블록이 사라져 나머지의 길이 열린다.','두 열쇠 · 두 길 · 되돌아오기 · 3층 창고 · 옥상'),
    ...
    ('상자 릴레이','상자를 밀어 단을 오르고(첫 단 위 상자를 둘째 단으로 밀어 두 단 다 오른다), 틈으로 떨어뜨려 아래층 발판으로 쓴다.','상자 계단 · 되돌아오기 · 탑 · 3층 창고'),
...
LEGEND = [
    ...
    ('n','나오는 발판 · 스위치','점선으로만 보이다가 스위치 a 를 밟으면 발판이 되어 그대로 남는다','#3f8f56'),
    ('m','나오는 발판 · 누름판','누름판 q 를 <b>밟고 있는 동안만</b> 발판. 내려서면 사라진다 — 위에 선 사람은 떨어진다 (M 은 p 쪽)','#2f6fb0'),
...
<header class="head">
  <div class="kicker">몰겜 · 세 번째 게임 · 기획 7판 — 손잡기 없는 판</div>
  <h1>넷이서</h1>
  ...
  <dl class="spec">
    ...
    <div><dt>6판에서</dt><dd>지하철 두 판 · 삭은 발판 · 상자가 무빙워크를 탄다 · 엔진 버그 아홉 · 판 여섯 고침</dd></div>
    <div><dt>7판에서</dt><dd><b>손잡기를 뺐다</b> · 스위치·누름판이 다른 데 발판을 낸다(n·M·m) · 판 여섯을 다시 짰다 · 사람끼리 부딪힌다 · 규약 8</dd></div>
  </dl>
</header>
...
<section>
  ...
  <h2>6판에서 고친 것 — 검토</h2>
  <p class="sub">「맵을 다 검토하고, 버그도 검토하고, 처음 점프할 때 발이 땅 밑까지 들어가는 것도」. 엔진 아홉(회귀 하나 포함), 판 여섯, 검사기 둘.</p>
  <h3>엔진</h3>
  {verbs([
    ...
    ('손잡기가 벽 모서리 1px 위 허공에 세웠다','올라선 사람이 설 자리는 몸이 4px 이상 바닥에 걸쳐야 한다. 상대 쪽 옆, 반대쪽 옆, 아니면 내 자리.','「옥상」 세 칸 벽 위에서 끌어올린 사람이 도로 떨어졌다. 7판에서 손잡기 자체를 뺐다 (14).'),
    ('출구 안에서 동료 머리 위에 서면 「안」이 아니었다','포탈 앞에 넷이 몰리면 누가 누구 머리 위에 서게 된다. 한 칸 반까지 안이다.','「움직이는 발판」 끝에서 넷이 모였는데 「2명 더」.'),
    ('(회귀) 남의 발을 세우는 계산이 사다리 꼭대기를 바닥으로 쳤다','첫 고침이 낳은 것. 사다리를 내려오는 동료가 내 화면에서 꼭대기에 붙어 안 내려왔다. 사다리 꼭대기는 그 계산에서 바닥이 아니다 — 시험으로 박아 뒀다.','3.4.1. 판 목록과 꾸러미가 달라져 옛 앱은 방에 못 들어온다 (규약 6).'),
  ])}
  <h3>판</h3>
  {verbs([
    ...
    ('탑 · 무빙워크 · 종점 — 손 닿는 자리에 다른 사람','손잡기는 옆 한 칸·아래 세 칸 안에서 가장 가까운 사람을 끌어올린다. 다른 사람이 그 안에 서 있으면 그쪽이 올라온다 — 두 칸 물러선다.','검사기가 곁 사람을 봤다 — 7판에서 손잡기를 뺀 뒤엔 해당 없다.'),
  ])}
  <h3>검사기</h3>
  {verbs([
    ...
    ('칸 검사기(solve.py) 규칙 다섯','열쇠는 닿으면 집는다 · 포탈은 지나가면 들어간다 · 내려뛰기는 발판을 뚫지 못한다 · 스위치·누름판은 셔터를 열거나 다른 데 발판을 낸다(7판) · 무빙워크 위 상자는 혼자 간다(ride).','어림값 검사가 엔진과 같은 말을 하게.'),
    ('넷 세상 검사(coop-net.mjs)','방장 1 + 손님 3, 세상 넷, 꾸러미를 50ms 지연으로 주고받는다. 손님이 자기 물리로 걷고, 방장 스냅샷(pack/unpack)으로 상자·열쇠·삭은 발판·나오는 발판을 받고, 밀기·출구 부탁이 방장을 거쳐 중계된다. 한 명씩 하는 동작(걷기·사다리·리프트·열쇠·포탈·누름판·나오는 발판)은 지연을 넣어도 넘어간다.','다만 봇은 한 명씩 움직인다 — 둘이 한 상자를 동시에 밀거나 사람 계단을 쌓는 진짜 동시 동작은 넷이 붙어야 안다.'),
  ])}
</section>
...
<section>
  ...
  <h2>7판 — 손잡기를 뺐다</h2>
  <p class="sub">「손잡기가 잘 티가 안 나. 빼는 방향으로 설계를 다 다시 해 줘. 맵도 다 검토하고. 트릭을 많이 넣어도 좋아 — 위 두 명이 트릭을 성공하면 아래 두 명의 발판이 생긴다거나.」</p>
  <h3>왜 뺐나</h3>
  {verbs([
    ('보이지 않았다','끌어올리는 순간 상대가 세 칸 위로 옮겨질 뿐이라, 실제로도 영상에서도 「저 사람 혼자 어떻게 올라갔지」로 보였다. 손·줄을 그려 넣어 봤지만(3.4.5) 동작 자체가 순간이동이라 한계였다.',''),
    ('판이 그것에 기댔다','세 칸 턱을 넘는 법이 늘 「어깨 → 손」 한 가지였다. 여섯 판이 같은 마디를 되풀이했다.','상자 계단 · 탑 · 3층 창고 · 옥상 · 무빙워크 · 종점.'),
    ('넷이 각자 올라가는 편이 협동답다','한 명이 남을 끌어올리는 대신, 한 명이 해낸 일이 <b>남의 길을 낸다</b>. 남은 사람도 자기 발로 간다 — 피코파크의 스위치·발판 방식.',''),
  ])}
  <h3>새 장치 셋</h3>
  {verbs([
    ('n — 스위치가 내는 발판','초록 점선 칸. 스위치 a 를 밟으면 발판이 되어 그대로 남는다. 넘어간 한 명이 밟아 남은 셋의 계단을 낸다.','옥상(옥상 스위치 → 1층 끝 계단 둘) · 무빙워크(벽 너머 스위치 → 벽 앞 발판)'),
    ('m — 누름판이 내는 발판','파란 점선 칸. 누름판 q 를 <b>밟고 있는 동안만</b> 발판이다. 밟던 사람이 내려서면 사라진다 — 위에 선 사람은 떨어진다. 믿고 서 있어야 한다.','3층 창고(한 명이 밟고 한 명이 오른다) · 종점(1층 무거운 상자가 누름판을 눌러 2층 발판을 낸다 — 층을 건너는 연결)'),
    ('M — 누름판 p 쪽','m 과 같다. 지금 쓰는 판은 없다 — 다음 판을 위해 엔진·검사기에 들어 있다.',''),
    ('사람끼리 부딪힘','같은 높이의 사람은 서로 통과하지 못하고, 뒤에서 밀면 앞 사람이 조금씩 밀려난다. 머리 위·어깨는 세로 관계라 안 민다.','3.4.3. 앞을 막고 선 사람은 밀어서 치운다.'),
  ])}
  <h3>판 여섯 — 전과 후</h3>
  {rows([
    ('1-2 상자 계단','둘째 단 네 칸: 사람 계단으로 둘, 남은 둘은 손잡기','둘째 단 세 칸. 첫 단 위 상자를 둘째 단으로 밀어 <b>두 단 다 상자로</b> 넷이 오른다'),
    ('3-1 탑','3층 칸막이·옥상 물탱크 세 칸: 어깨 → 손 ×2','두 칸으로 낮춰 <b>각자 뛰어넘는다</b>. 협동은 지그재그 사다리와 노란 열쇠(발밑이 사라진다)에'),
    ('3-2 3층 창고','3층 단 위 열쇠: 어깨 → 손','한 명이 <b>누름판 q 를 밟고 서 있으면</b> 단 옆에 발판 m. 다른 한 명이 딛고 올라 열쇠. 밟는 사람이 먼저 내려오면 발판이 사라진다'),
    ('3-4 옥상','옥상 세 칸 벽: 어깨 → 손 · 1층 끝 단 네 칸: 상자 + 어깨 + 손 ×2','옥상 벽은 두 칸. <b>옥상 스위치를 밟으면 1층 끝에 계단 둘(n)</b> — 위에서 한 사람이 해낸 일이 밑에서 기다리던 둘의 길'),
    ('4-1 무빙워크','세 칸 벽: 어깨 → 손 ×2','<b>어깨로 한 명만</b> 넘긴다. 그가 저편 스위치를 밟으면 이쪽 벽 앞에 발판(n) — 남은 셋이 딛고 넘는다. 어깨 내준 사람은 발판 자리에서 비켜 선다'),
    ('4-2 종점','2층 세 칸 벽: 어깨 → 손 ×2','<b>1층 무거운 상자를 누름판 q 에 올리면 2층 벽 앞에 발판(m)</b>. 층을 건너는 연결 — 상자를 치우면 발판이 사라진다'),
  ])}
  <h3>확인</h3>
  {verbs([
    ('칸 검사','열네 판 {STEPS_ALL}걸음. 스위치·누름판이 발판을 내는 규칙을 검사기가 안다 — 누름판이 풀리면 발판이 사라진다.',''),
    ('실제 엔진','같은 풀이를 넷이 키를 눌러 열네 판 다 깼다 (위 12 표). 손잡기 걸음은 봇에서도 뺐다.','한 명씩 · 인형 세상. 같은 칸에 넷이 차례로 내려서면 머리 위에 쌓이므로 내려선 사람은 바로 비켜 서는 걸음을 넣었다.'),
    ('단위 시험 152','발판 n·M·m 이 조건에 따라 나오고 사라지는 것, 밟던 사람이 내려서면 위 사람이 3층 바닥까지 떨어지는 것, 사람끼리 막히고 밀리는 것.',''),
    ('규약 8','옛 앱은 이 발판을 모른다 — 섞이면 한쪽만 밟고 선다. 방에 못 들어오게 규약을 올렸다.','3.5.0'),
    ('넷이 동시에 노는 영상을 만들다 잡은 것','무빙워크에 실려 상자에 붙은 사람이 반대쪽으로 걸어도 못 벗어났다 — 벨트가 끌어다 붙이는 걸 「내가 상자를 민다」로 쳐서 매 프레임 상자에 다시 붙었다. 실려 가는 몫은 걸음과 따로 더하고, 상자에 닿으면 옆에 선다. 한 명씩 하는 검사에서는 안 나왔다 — 둘이 같은 벨트에 내려 하나가 기다리는 사이 벨트가 끌어갔을 때만.','3.5.1. 시험으로 박았다.'),
    ('포탈에서 도로 튕겨 갔다','포탈은 0.3초 뒤 다시 발동했다 — 저편에 내려선 채 잠깐 서 있으면 그 자리에서 되돌아갔다. 칸을 떠났다가 다시 들어올 때만 탄다 (사람도 상자도).','3.5.1. 한 명씩 하는 검사는 도착하자마자 걸어 나가서 못 봤다.'),
  ])}
  <h3>동시에 노는 영상</h3>
  <p class="sub"><code>test/coop-cobot.mjs</code> — 방장 1 + 손님 3, 세상 넷을 프레임마다 함께 굴리고, 걸음이 서로 안 걸리는 한 넷이 <b>동시에</b> 움직인다 (어깨·밀기·열쇠 같은 협동 마디만 차례로). 화면은 실제처럼 <b>네 사람 시점 2×2</b> — 각자 자기 카메라·글자판·화면 밖 친구 화살표. 같은 칸에 내리는 뛰기는 한 번에 하나, 내릴 자리에 남이 서 있으면 비킬 때까지(3초) 기다린다.</p>
</section>
...
const COL = {{'#':'#141210','=':'#6b665c','H':'#8a7a5a','|':'#8a7a5a','-':'#2f6fb0','^':'#d02f22','x':'#6f4a2c','X':'#4f3320',
  ...
  'u':'#2f6fb0','U':'#2f6fb0','w':'#2f6fb0','W':'#2f6fb0','O':'#8a5bb5','{{':'#d97b1f','}}':'#d97b1f','S':'#2f9c9c','v':'#b8912a','~':'#b8912a','>':'#2f6fb0','<':'#2f6fb0','n':'#3f8f56','M':'#2f6fb0','m':'#2f6fb0'}};
...
open('coop-plan7.html','w').write(HTML); print('wrote', len(HTML))
