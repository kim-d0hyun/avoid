...
<header class="head">
  <div class="kicker">몰겜 · 세 번째 게임 · 기획 9판 — 환승역과 새 장치 셋</div>
  <h1>넷이서</h1>
...
<section>
  ...
  {verbs([
    ...
    ('환승역 「마지막 열차」 144×36 · 넷 모두','둘이 발전기(무거운 상자)를 밀어 누름판에 세우면 셔터 → 끊긴 선로 뒤 담은 어깨로 한 명만 먼저 → 그 사람이 스위치를 밟아 남은 셋의 계단(n) → 넷이 집결판에 모여 체크포인트, 집결문 → 사다리로 2층 신호실 → 삭은 통로 한 명씩 → 시한 스위치와 10초 문 → 점멸 통로 → 끝의 구조 레버 → 접이식 사다리로 전원 하층 승강장의 출구에. 정적 풀이 67걸음.','역할이 네 번 바뀐다 — 미는 둘, 먼저 넘는 하나, 스위치로 계단을 내는 하나, 레버를 밟는 선두.'),
    ('깰 수 있다','칸 검사(run.py) → 순차 인형 풀이(coop-play) → 사람 충돌과 편도 2프레임 지연을 켠 독립 월드 동시 봇(coop-cobot BUMP=1) 66초 클리어. 셋 다 통과.','동시 봇에서 처음엔 사람 탑 앞에 넷이 한꺼번에 몰려 실패했다 — 지형이 아니라 순서. 먼저 오를 사람과 받침만 건너고 나머지는 스위치가 켜진 뒤 합류하도록 풀이를 바꿨다.'),
...
function draw(cv, st) {{
  ...
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {{
    ...
    if ('apqctl'.includes(ch)) {{ g.fillStyle = paper; g.fillRect(X+2, Y+T*0.55, T-4, T*0.3); }}
  }}
