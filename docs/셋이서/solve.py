...
class Run:
    ...
    def ride(self, box, x1):
        """컨베이어 위의 상자는 혼자 간다 — 그 방향의 '>' '<' 칸을 따라, 막히는 데까지."""
        bx, by, w = self.boxes[box]
        belt = self.g.get(bx, by + 1)
        if belt not in '<>': raise Bad(f'{box} 가 컨베이어 위에 없다 ({bx},{by}) 밑은 {belt}')
        ...
        self.boxes[box] = (x1, by, w); self.say('', f'{box} 가 컨베이어에 실려 {x1}칸까지 간다')
