# -*- coding: utf-8 -*-
"""풀이를 한 걸음씩 지형에 대 본다 — 셋이서 판.

사람은 한 칸 폭 두 칸 높이. 점프는 두 칸 위 네 칸 옆. 어깨 한 명당 한 칸 —
셋이니 어깨는 최대 두 명(위로 네 칸)까지다. 무거운 상자 X 는 둘이 붙어야 밀린다,
그러면 남는 사람은 하나뿐이다. 그게 셋이서의 규칙이다.
"""
JUMP_UP, JUMP_ACROSS, SPRING_UP = 2, 4, 5
CREW = '123'
HELP_MAX = len(CREW) - 1        # 어깨는 최대 두 명
...
class Run:
    def __init__(self, g, end='all'):
        ...
        self.p = {ch: g.find(ch)[0] for ch in CREW}
        self.boxes = {}
    ...
    def boost(self, who, x1, y1, on):
        x, y = self.p[who]
        if len(on) > HELP_MAX: raise Bad(f'어깨는 최대 {HELP_MAX}명이다 — 셋이서니까 ({len(on)}명 적었다)')
        if who in on: raise Bad(f'{who} 가 자기 어깨를 밟는다')
        for o in on:
            ox, oy = self.p[o]
    ...
    def ride(self, box, x1):
        """컨베이어 위의 상자는 혼자 간다 — 그 방향의 벨트 칸을 따라, 막히는 데까지. 분기 벨트 J·j 는 누름판 p·q 가
        눌린 동안 오른쪽, 아니면 왼쪽(회수)으로 돈다. 벨트 위 누름판 p·q 에 든 상자는 거기 선다 (상자 정차대)."""
        bx, by, w = self.boxes[box]
        step = self.belt_dir(self.g.get(bx, by + 1))
        if not step: raise Bad(f'{box} 가 컨베이어 위에 없다 ({bx},{by}) 밑은 {self.g.get(bx, by + 1)}')
        if (x1 - bx) * step <= 0: raise Bad(f'{box} 는 컨베이어가 {"오른쪽" if step > 0 else "왼쪽"}으로 간다 — {x1}칸은 반대다')
        cx = bx
        while cx != x1:
        ...
        self.boxes[box] = (x1, by, w); self.say('', f'{box} 가 컨베이어에 실려 {x1}칸까지 간다' + (' — 누름판에 선다' if parks else ''))
    ...
    def tower(self, who, on):
        ...
        x, y = self.p[who]
        if len(on) + 1 < len(CREW): raise Bad(f'탑은 전원이 쌓아야 한다 — {len(on) + 1}명')
        for o in on:
            ox, oy = self.p[o]
            if abs(ox - x) > 1 or oy != y: raise Bad(f'{o} 가 {who} 옆에 없다 ({ox},{oy}) vs ({x},{y})')
        n = len(on)
    ...
    def stairs(self, who, x1, y1, on):
        ...
        한 명이 한 칸씩 더 준다 — 어깨(boost)와 높이는 같지만, **옆으로도** 그만큼 더 간다."""
        x, y = self.p[who]
        if len(on) > HELP_MAX: raise Bad(f'사람 계단은 최대 {HELP_MAX}명이다')
    ...
    def finish(self):
        ...
        if self.end == 'all' and len(inside) < len(CREW):
            out = [w for w in self.p if w not in inside]
            raise Bad(f'셋이 다 출구에 없다 — {out} 는 {[self.p[w] for w in out]}')
        if self.end == 'one' and not inside: raise Bad('아무도 출구에 없다')
        self.say('·'.join(inside), '출구 포탈에서 ⌥↑ — ' + ('셋이 다 모였다' if self.end == 'all' else '한 명이 닿았으니 끝'))
