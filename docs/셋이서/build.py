# 셋이서 기획 페이지 — PLAN.md + 판 열둘의 지도(글자 한 칸)를 HTML 로. python3 build.py → plan3.html
...
legend = ('<p class="sub">글자 하나가 한 칸. <b style="color:#141210">#</b> 땅·벽 · <b style="color:#6b665c">=</b> 선반 · <b style="color:#8a7a5a">H</b> 사다리 · '
          ...
          '<b style="color:#2f6fb0">u U w W</b> 포탈 · <b style="color:#8a5bb5">O</b> 출구 · <b style="color:#d97b1f">{ }</b> 통 · <b>1 2 3</b> 시작 자리.</p>')
HTML = f"""<title>셋이서</title>
{CSS}{EXTRA}
<header>
  <div class="kicker">몰겜 · 넷째 게임 후보 · 기획 1판</div>
  <h1>셋이서</h1>
  <p class="lead">넷이서와 같은 타일·같은 물리로 만든 <b>3인 협동 판 열둘</b>. 손이 하나 모자란다는 전제 위에 세웠다 — 무거운 상자를 둘이 밀면 남는 건 하나, 어깨를 둘이 받치면 올라가는 것도 하나. 열두 판 풀이 486걸음을 칸 검사기로 확인했다. 실제 엔진 검사와 구현은 아직이다 (6장).</p>
</header>
<main>
{md(plan)}
<h2>판 열둘 — 지도</h2>
...
<footer><div>자료는 리포 <code>docs/셋이서/</code> — <code>stages.py</code>(판) · <code>solve.py</code>(3인 검사기) · <code>PLAN.md</code>. 확인: <code>python3 run.py</code> → 12판 · 486걸음.</div></footer>
"""
