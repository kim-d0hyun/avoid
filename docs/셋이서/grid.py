# -*- coding: utf-8 -*-
"""판을 조각으로 쌓는다. 셋이서 — 사람이 셋이다. 크기는 판마다 다르다.

한 칸 42px. 화면 하나가 36칸 × 22줄.

  #  땅·벽        =  선반(밑에서 통과)   H  사다리      ^  가시        S  스프링
  x  상자(1인)    X  무거운 상자(2인 — 셋 중 둘이 붙으면 하나가 남는다)
...
class Grid:
    ...
    def blink(self, x0, x1, y):
        return self.fill(x0, y, x1, y, '~')
    def rot(self, x0, x1, y):
        return self.fill(x0, y, x1, y, 'v')
    def belt(self, x0, x1, y, ch='>'):
        return self.fill(x0, y, x1, y, ch)
    def spikes(self, x0, x1, y):
        return self.fill(x0, y, x1, y, '^')
    def pit(self, x0, x1, y, depth=1):
        """땅에 뚫린 구멍 — 바닥에 가시."""
        self.fill(x0, y, x1, y + depth, '.'); return self.spikes(x0, x1, y + depth)
    ...
    def spawn(self, x, y, gap=2):
        for i, ch in enumerate(CREW): self.put(x + i * gap, y, ch)
        return self
