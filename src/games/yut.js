...
export function nextOf(at, viaShort = false) {
  ...
  // **방에 섰다 떠나면** 출발 쪽 대각선으로 나간다 (제일 짧은 길).
  // **그냥 지나가면** 타고 있던 대각선을 그대로 따라 왼쪽 아래 모(15)로 나간다 —
  // 실제 윷놀이의 규칙이 그렇고, 이걸 안 갈라 두는 바람에 23·24 밭은 **한 번도 안 밟혔다**
  // (판에는 그려져 있는데 아무도 못 가는 길이었다).
  if (at === CENTER) return viaShort ? 28 : 23;
  if (at === 23) return 24;
  if (at === 24) return 15;
  if (at === 26) return 27;
...
export function spotOf(at) {
  ...
  if (at >= 1 && at <= RING) {
    ...
    //
    // **한 칸씩 밀려 있었다.** k 를 0부터 세는 바람에 1번 밭이 출발 귀퉁이에 그려졌고,
    // 말이 죄다 한 칸 뒤에 서 있었다 — 집에서 걸(3)을 놓으면 출발 귀퉁이에서 두 칸만
    // 간 것처럼 보인다(「걸인데 왜 두 칸 가지?」). 더 나쁜 건 **모가 어긋난 것**이다.
    // 지름길을 타는 밭은 5·10 인데 귀퉁이에 그려진 건 6·11 이라, 지름길이 모가 아닌
    // 밭에서 대각선으로 새는 것처럼 보였다. 첫 걸음이 귀퉁이에서 **한 칸 간 자리**다.
    const side = Math.floor((at - 1) / 5), k = (((at - 1) % 5) + 1) / 5;
